...
const ListLabel = styled(Paragraph)(({ theme, mode }: LabelProps) => {
	return {
		fontSize: "12px",
		marginTop: "20px",
		marginLeft: "15px",
...
const InternalLink = styled(Box)(({ theme }: any) => ({
	"& a": {
		...ExtAndIntCommon,
		color: theme.palette.text.primary,
	},
...
const StyledText = styled(Span)<StyledTextProps>(({ mode }: StyledTextProps) => ({
	fontSize: "0.875rem",
	paddingLeft: "0.8rem",
	display: mode === "compact" ? "none" : "block",
}));
...
const MatxVerticalNav: React.FC<Props> = ({ items }) => {
